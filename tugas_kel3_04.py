...
for key,value in dataset.items():
	for o in range(6):
		new_value = (value[o]/max(value))*100
		dataset.update({key : new_value})

# fitur tetsing dari testing_haar
for key,value in testing.items():
	for o in range(6):
		new_value = (value[o]/max(value))*100
		testing.update({key : new_value})
...
for key,value in testing.items():
	...
	for key2,value2 in dataset.items():
		for o in range(6):
			selisih_perMahasiswa[o] = value[o]-value2[o]
		for p in range(6):
			total_perMahasiswa[z] += selisih_perMahasiswa[p]
		if(total_perMahasiswa[z]<temp_score):
			temp_score = total_perMahasiswa[z]
			best_name = key2[y]
		y+=1
		z+=1
	print(best_name)
